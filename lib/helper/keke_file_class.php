...
class keke_file_class {
	...
	static function formatFileSize($filepath) {
		...
		$_size = self::getByteSize($filepath);
		if (!$_size || $_size < 0) {
		...
		$formatted = number_format($_size,2);
		return array($_size, $formatted . " " . $unit, array($formatted, $unit));
	}
	static function getSize($filepath){
		if(substr($filepath, 0,4) == 'http'){
			$filepath  = self::remote_filesize($filepath);
		}else{
			$filepath  = self::formatFileSize($filepath);
		}
		return $filepath[1];
	}
	static function getByteSize($filepath) {
		$bytesize = @filesize($filepath);
		if (false !== $bytesize && $bytesize >= 0) {
			return $bytesize;
		}
	}
	static function remote_filesize($uri)
	{
		ob_start();
		$ch = curl_init($uri); 

...
EOT;
	...
	public static function getAllEmail($email){
		$arrEmail=db_factory::query("select * from ".TABLEPRE."witkey_email");
	...
	public static function getUserInfoByMobile($mobile){
		return db_factory::get_one("SELECT * FROM `".TABLEPRE."witkey_space` WHERE mobile = '".$mobile."'");
	}
	public static function getUserInfoByUsername($username){
		return db_factory::get_one("SELECT * FROM `".TABLEPRE."witkey_space` WHERE username = '".$username."'");
	}
	public static function getUserInfoByEmail($email){
		return db_factory::get_one("SELECT * FROM `".TABLEPRE."witkey_space` WHERE email = '".$email."'");
	}
	...
	public static function getGetPwdLogByAuthsid($authsid){
		return db_factory::get_one("SELECT * FROM `".TABLEPRE."witkey_get_password_log` WHERE authsid = '{$authsid}'");
	}
	public static function updateGetPwdStatusByGetUid($getuid,$status){
		return db_factory::execute("UPDATE `".TABLEPRE."witkey_get_password_log` SET `valid_status`='{$status}' WHERE (`get_uid`='{$getuid}')");
	}
	public static function getPwdAuth($authsid,$encrypteuid){
		$return = db_factory::get_one("SELECT * FROM `".TABLEPRE."witkey_get_password_log` WHERE authsid = '{$authsid}' AND encrypteuid = '{$encrypteuid}' AND valid_status = 2 ORDER BY expire_time DESC LIMIT 1");

...
class keke_user_login_class {
	...
	function user_login($account, $password, $code = null, $login_type = 0,$is_oauth=null) {
		global $_lang,$_K;
		...
		switch ($this->_sys_config ['user_intergration']) {
			case "1" :
				...
				if ($user_info ['password'] !==  md5($password )) {
					...
					$url="index.php?do=login";
					if($is_oauth){
						$this->show_msg ( '用户名或密码错误，请检查账号是否输入正确', 3,$_SERVER['HTTP_REFERER'],1 );
					}else{
				...
				break;
			case "2" :
				$accout_type != 'username'&&$this->show_msg ( $_lang ['integrated_model_nust_use_name'],3,"index.php?do=login");
	...
	function valid_moble_auth() {
		...
		$sql =  "
					SELECT
						a.*
					FROM
						`".TABLEPRE."witkey_auth_mobile` AS a
					LEFT JOIN `".TABLEPRE."witkey_auth_record` AS b ON a.uid = b.uid
					AND b.auth_code = 'mobile'
					WHERE
						a.mobile = '{$this->_account}'
					AND a.auth_status = '1'
					AND b.auth_status = '1'
				";
	...
	public function save_user_info($user_info,$account, $ckb_cookie = 1, $login_type = 0,$auto_login=0, $oauth_login = 0) {
		...
		if ($auto_login == '1'){
			...
			$c[2] =base64_encode($user_info ['uid'].'|'.$_SERVER["HTTP_USER_AGENT"].'|'.$account);
            $token = serialize($c);
			setcookie('keke_auto_login', $token, time()+3600*24*30);
			db_factory::execute("update ".TABLEPRE."witkey_space set token = '".$token."' where uid = '".$user_info ['uid']."'");
		}
		if ($_K ['refer']) {
			$r = $_K ['refer'];
		} else {

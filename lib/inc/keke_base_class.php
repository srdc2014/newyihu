...
class keke_base_class {
	...
	public static function set_star($str, $start, $end, $start_num = 3, $preg_str = "*") {
		...
		if (strlen ( $str ) <= ($start + $end)) {
			return substr($str, 0,2).$replace_str;
		}
		$start_str = mb_substr ( $str, 0, $start, CHARSET );
		$tmp_str = mb_substr ( $str, 0, - $end, CHARSET );
		$end_str = mb_substr ( $str, mb_strlen ( $tmp_str ), mb_strlen ( $str ) );
		$replace_str = str_repeat ( $preg_str, $start_num );
	...
	static function randomkeys($length,$number = false) {
		$key = null;
		if($number){
			$pattern = '1234567890'; 
		}else{

...
class keke_table_class {
	...
	private function checkDelRights($field,$fieldval){
		...
		$sql = "SELECT COLUMN_NAME FROM information_schema.`COLUMNS` WHERE TABLE_SCHEMA = '".DBNAME."' AND TABLE_NAME = '".TABLEPRE.$this->_table_name."' AND COLUMN_NAME LIKE '%uid%'";
		$columns = db_factory::query($sql);
		$arrSize = count($columns);
		if($arrSize){
			$count = 0;
			foreach ($columns as $key=>$val){
				if(db_factory::get_count(sprintf("select * from %s where %s='%s' and %s = '%d'",TABLEPRE.$this->_table_name,$field,$fieldval,$val['COLUMN_NAME'],$s_uid))){
					$count++;
				}
			}
			if($count > 0){

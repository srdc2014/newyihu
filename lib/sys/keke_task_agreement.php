...
abstract class keke_task_agreement {
	...
	public function agreement_stage_one($user_type) {
		...
		if ($agree_status == '1') {
			...
			$msg_obj->send_message ( $this->_seller_uid, $this->_seller_username, "agreement", $_lang['deliver_agreement_sign'], $s_arr, $this->_seller_contact ['email'], $this->_seller_contact ['mobile'],1 ); 
			$msg_obj->send_message ( $this->_buyer_uid, $this->_buyer_username, "agreement",  $_lang['deliver_agreement_sign'], $s_arr, $this->_buyer_contact ['email'], $this->_buyer_contact ['mobile'] ,1); 
		} else {
	...
	public function upfile_confirm($file_ids) {
		...
		$msg_obj->send_message ( $this->_buyer_uid, $this->_buyer_username, "agreement_file", $_lang['agreement_files_submit'], $v_arr, $this->_buyer_contact ['email'], $this->_buyer_contact ['mobile'] ,1); 
		if($res){
			return true;
		}else{

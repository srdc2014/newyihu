...
abstract class keke_task_class {
	...
	public function get_task_file() {
		return kekezu::get_table_data ( "*", "witkey_file", " obj_type = 'task' and task_id = '" . $this->_task_id . "'", "", "", "", "file_id", 3600 );
	}
	...
	public function check_if_can_hand() {
		...
		if (! $this->_priv ['work_hand'] ['pass']) {
			return '没有交稿权限'.$this->_priv ['work_hand'] ['notice'].'<a href="index.php?do=user&view=account&op=auth">立即认证</a>';
		}
		$this->process_can();
		...
		if (! empty ( $item_arr ) && is_array ( $item_arr )) {
			foreach ( $item_arr as $v ) {
				if ($item_config [$v] ['item_code'] == 'top') {
					time () < intval ( $time_arr [top] ) && $item_config [$v] ['small_pic'] and $str .= "<img src='" .  imageClass::absUrl($item_config [$v] ['small_pic'] )  . "'>";
				} elseif ($item_config [$v] ['item_code'] == 'urgent') {
					time () < intval ( $time_arr [urgent] ) && $item_config [$v] ['small_pic'] and $str .= "<img src='" . imageClass::absUrl($item_config [$v] ['small_pic'] ) . "'>";
				} else {
					$item_config [$v] ['small_pic'] and $str .= "<img src='" . imageClass::absUrl($item_config [$v] ['small_pic'] ) . "'>";
				}
			}
		}
		return $str;

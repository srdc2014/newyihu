...
class keke_order_class {
	...
	public static function create_user_charge_order($order_type, $pay_type, $money, $obj_id = null, $pay_info = '', $order_status = 'wait', $uid = '', $username = '') {
		...
		if ($create) {
			$order_id = self::create_order_charge($order_type, $pay_type, null, $obj_id, $uid, $username, $money, $order_status, $pay_info);
		}
		return $order_id;
	}
	public static function update_order_charge($out_trade_no,$pay_money){
		$order_info = db_factory::get_one ( sprintf ( "select * from %switkey_order_charge where out_trade_no ='%s'", TABLEPRE, $out_trade_no ) );
		if($order_info ['order_status'] == 'wait'){
			db_factory::execute ( sprintf ( " update %switkey_order_charge set pay_money='%.2f',pay_time='%s' where order_id='%d'", TABLEPRE, $pay_money, time (), $order_info ['order_id'] ) );
		}
		return $order_info ['order_id'];
	}
	public static function create_order_charge($order_type,$pay_type,$return_order_id,$obj_id,$uid,$username,$pay_money,$order_status,$pay_info,$out_trade_no,$pay_time =NULL,$attach = NULL){
		$order_obj = new Keke_witkey_order_charge_class ();
		$order_obj->_order_id = null;
		$order_obj->setOrder_type ( $order_type );
		$order_obj->setPay_type ( $pay_type );
		$return_order_id and $order_obj->setReturn_order_id($return_order_id);
		$order_obj->setObj_id ( $obj_id );
		$order_obj->setUid ( $uid );
		$order_obj->setUsername ( $username );
		$order_obj->setPay_money ( $pay_money );
		$order_obj->setOrder_status ( $order_status );
		$order_obj->setPay_info ( $pay_info );
		$order_obj->setPay_time ( $pay_time);
		$order_obj->setOut_trade_no($out_trade_no);
		$order_obj->setAttach($attach);
		return $order_obj->create_keke_witkey_order_charge ();
	}

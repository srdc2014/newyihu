...
abstract class keke_task_release_class {
	...
	public function notify_user($task_id, $task_status = '2') {
		...
		$status_arr = call_user_func ( array (
				$model_code . "_task_class",
				'get_task_status' 
		) ); 
		$url = '<a href="' . $_K ['siteurl'] . '/index.php?do=task&id=' . $task_id . '"  target="_blank">' . $task_obj->getTask_title () . '</a>';
		$v = array (
				$_lang ['task_title'] => $task_obj->getTask_title (),
				$_lang ['task_id'] => $task_id,
				$_lang ['task_link'] => $url,
				$_lang ['task_status'] => $status_arr [$task_status],
				$_lang ['start_time'] => date ( 'Y-m-d H:i:s', $task_obj->getStart_time () ),
				$_lang ['hand_work_timeout'] => date ( 'Y-m-d H:i:s', $task_obj->getSub_time () ),
				$_lang ['choose_timeout'] => date ( 'Y-m-d H:i:s', $task_obj->getEnd_time () ) 
		);
		keke_msg_class::notify_user ( $this->_uid, $this->_username, "task_pub", $_lang ['pub_task_notice'], $v, 1 );
	}
	...
	public function update_task_info($task_id, $obj_name) {
		...
		if (in_array ( $this->_model_info ['model_code'], array (
				'sreward',
				'mreward',
				'preward' 
		) )) {
	...
	public function onekey_mode_format($data) {
		return array (
				"txt_title" => $data ['task_title'],
				"tar_content" => $data ['task_desc'],
				"indus_id" => $data ['indus_id'],
				"indus_pid" => $data ['indus_pid'],
				"step1" => '1',
				"txt_mobile" => $data ['contact'] 
		);
	}
	public function check_pub_priv($url = '', $output = "normal") {
		...
		if ($this->_priv ['pass']) {
			...
			return $this->_priv ['notice'] . $_lang ['not_rights_pub_task'] . '<a href="index.php?do=user&view=account&op=auth">立即认证</a>';
		}
	}

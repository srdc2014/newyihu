...
class keke_auth_bank_class extends keke_auth_base_class {
	...
	public function auth_confirm($auth_info, $user_get_cash) {
		...
		$ac_url = $basic_config ['website_url']."index.php?do=user&view=account&op=auth&code=" . $this->_auth_code . "&intBankAid=" . $intBankAid . "#userCenter";
	...
	public function authFail($auth_info, $user_get_cash) {
		...
		keke_msg_class::notify_user ( $uid, $username, 'auth_fail', $this->auth_lang () . $_lang ['fail'], $v_arr,2 );
	}
}
...
switch ($op) {
	...
	case "quote" : 
		...
		if (isset($formhash)&&kekezu::submitcheck($formhash)){
			...
			$work_frm ['work_hidden'] = $work_hidden;
	...
	case "matchquote" : 
		...
		if (isset($formhash)&&kekezu::submitcheck($formhash)){
			...
			$resText = $objTask->work_bid ( $mobile,$price,$tar_content,$cycle,$work_hidden );
			if($resText === true){
	...
	case 'taskHost':
		if (isset($formhash)&&kekezu::submitcheck($formhash)){
			$zfpwd = trim($zfpwd);
			$strPwd = keke_user_class::get_password ( md5($zfpwd), $gUserInfo['rand_code'] );
			if($gUserInfo['sec_code'] != $strPwd){
				$tips['errors']['zfpwd'] ='支付密码错误';
				kekezu::show_msg ( $tips, null, null, NULL, 'error' );
			}
			...
			if($strText === true){
				...
				kekezu::show_msg ( $strText, 'index.php?do=task&id='.$taskId, 3, NULL, 'fail' );
			}
		}else{
			$twoPassword = keke_user_class::get_password ( $gUserInfo['password'], $gUserInfo['rand_code'] );
	...
	case "planquote" : 
		if(0 === $gUid){
			...
			if (isset($formhash)&&kekezu::submitcheck($formhash)){
				...
				$resText = $objTask->bid_hand ( $quote,$cycle,$district,$message,$plan_amount, $start_time, $end_time, $plan_title ,'',$work_hidden);
				unset($objTask);

...
$arrServerNav = db_factory::get_table_data('*','witkey_model',' model_status = 1 and model_type = "shop" ',' listorder asc','','','model_id',3600);
    ...
    if($t=='1'){
		...
		if($m !== 0){
			foreach($arrCaseLists as $k=> $v){
				$arrTaskSql = 'select model_id,task_id,task_title,work_num,uid,username,task_pic,focus_num from '.TABLEPRE."witkey_task where task_id = '{$v['obj_id']}'";
		...
		if($s !== 0){
			foreach($arrCaseLists as $k=> $v){
				$arrTaskSql = 'select model_id,service_id,title,sale_num,uid,username,pic,focus_num from '.TABLEPRE."witkey_service where service_id = '{$v['obj_id']}'";
				$arrTaskInfo =  db_factory::get_one($arrTaskSql);

...
switch ($step) {
	case 'step1':
		if (isset($formhash)&&kekezu::submitcheck($formhash)) {
			...
			if($orderId){
				...
				$msg_obj->send_message($arrSellerInfo['uid'], $arrSellerInfo['username'], 'gy_order_notice', '雇佣订单消息',$v_arr,1);
	...
	case 'step5':
		if($arrServiceOrderInfo['workfile']){
			$workfiles = explode(',', $arrServiceOrderInfo['workfile']);
			if($workfiles && is_array($workfiles)){
				$workfile = array();
				foreach ($workfiles as $k=>$v){
					if(intval($v) > 0){
						$workfile[$k] = intval($v);
					}
				}
				if($workfile && is_array($workfile)){
					$fileids = implode(',', $workfile);
					$arrFileLists = db_factory::query('select file_name,save_name from '.TABLEPRE.'witkey_file where file_id in('.$fileids.')');
				}
			}
		}
		if(isset($action)){

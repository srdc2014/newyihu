<?php
	...
	if($arrServiceOrderInfo['file_ids']){
		$workfiles1 = explode(',', $arrServiceOrderInfo['file_ids']);
		if($workfiles1 && is_array($workfiles1)){
			$workfile1 = array();
			foreach ($workfiles1 as $k=>$v){
				if(intval($v) > 0){
					$workfile1[$k] = intval($v);
				}
			}
			if($workfile1 && is_array($workfile1)){
				$fileids = implode(',', $workfile1);
...
switch ($step) {
	case 'step1':
		if($gUid == $id){
			kekezu::show_msg('不能对自己进行雇佣操作','index.php',3,'非法操作','warning');
		}
		break;
	case 'step2':
		if(isset($action)){

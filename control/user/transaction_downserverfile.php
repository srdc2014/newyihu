 ...
 */
if($obj_id){
    $serviceInfo = db_factory::get_one('select * from '.TABLEPRE.'witkey_service where service_id='.$obj_id);
    $file_path = $serviceInfo['file_path'];
    $arrFile_path = explode('|',$file_path);
    foreach($arrFile_path as $v){
        $info = db_factory::query ( 'select file_name,save_name from ' . TABLEPRE . 'witkey_file where save_name ="' . $v . '"' );
        $arrFileLists[] = $info[0];
    }

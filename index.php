...
if ($gUid) {
    ...
    $indentity = db_factory::get_one("select `indentity` from ".TABLEPRE."witkey_space where uid = '$gUid'");
    $userUrl = 'gz';
    if ($indentity['indentity']) $userUrl = 'wk';
}
$isMobile = CommonClass::isMobile();
$isMobile and define('IS_MOBILE', TRUE) or define('IS_MOBILE', FALSE);
require 'control/' . $do . '.php';
if ($_K['theme'] && file_exists('tpl/default/theme/' . $_K ['theme'] . '/' . $do . '.htm')) {
    require $kekezu->_tpl_obj->template('tpl/default/theme/' . $_K ['theme'] . '/' . $do);
} else {

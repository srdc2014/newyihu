...
class WxPayApi
{
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url($WxPayCfg->_notify_url);
		}
		$inputObj->SetAppid($WxPayCfg->_appid);
		$inputObj->SetMch_id($WxPayCfg->_mchid);
	...
	{
		$WxPayCfg = new WxPayCfg();
		if($WxPayCfg->_report_levenl == 0){
			return;
		} 
		if($WxPayCfg->_report_levenl == 1 &&
	...
	{		
		...
		$WxPayCfg = new WxPayCfg();
		if($WxPayCfg->_curl_proxy_host != "0.0.0.0" 
			&& $WxPayCfg->_curl_proxy_port != 0){
			curl_setopt($ch,CURLOPT_PROXY, $WxPayCfg->_curl_proxy_host);
			curl_setopt($ch,CURLOPT_PROXYPORT, $WxPayCfg->_curl_proxy_port);
		}
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, $WxPayCfg->_sslcert_path);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, $WxPayCfg->_sslkey_path);
		}

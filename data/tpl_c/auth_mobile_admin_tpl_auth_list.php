<?php keke_tpl_class::checkrefresh('auth/mobile/admin/tpl/auth_list', '1457949939' );?><!DOCTYPE HTML>
...
<div class="page_title"><h1><?php echo $_lang['mobile_auth_list'];?></h1></div>
<!--页头结束-->
...
<td><input type="text" class="txt" name="w[mobile_a_id]" 	value="<?php echo $w['mobile_a_id'];?>" onkeyup="clearstr(this);"></td>
...
<div class="box list">
    	<div class="title"><h2><?php echo $_lang['mobile_auth_list'];?></h2></div>
        <div class="detail">
...
<th><?php echo $_lang['mobile'];?></th>
<th><?php echo $_lang['mobile_code'];?></th>
<!--	<th><?php echo $_lang['cash'];?></th>-->
<th><?php echo $_lang['auth_time'];?></th>
<th><?php echo $_lang['auth_status'];?></th>
<!--<th><?php echo $_lang['operate'];?></th>-->
          </tr>
<?php if(is_array($mobile_arr)) { foreach($mobile_arr as $key => $value) { ?>
<tr class="item">
<td><input type="checkbox" name="ckb[]" id="cbk_selected" value="<?php echo $value['mobile_a_id'];?>"></td>
<td><?php echo $value['mobile_a_id'];?></td>
<td><?php echo $value['username'];?></td>
<td><?php echo $value['mobile'];?></td>
<td><?php echo $value['valid_code'];?></td>
<!--<td><?php if($value['cash']) { ?><font color="#ff7700"><?php  echo keke_curren_class::output(floatval($value[cash]),-1);  ?></font><?php } else { ?><?php echo $_lang['free'];?><?php } ?></td>-->
<td><?php if($value[end_time]){echo date('Y-m-d',$value[end_time]); } ?>&nbsp;</td>
 
<td>
<?php if($value['auth_status']==1) { ?>
<font color="green"><?php echo $_lang['passed'];?></font>
<?php } elseif($value['auth_status']==2) { ?>
<font color="red"><?php echo $_lang['nopassed'];?></font>
<?php } else { ?>
认证中
<?php } ?>
<!--
<?php if($value['auth_status']==0||$value['auth_status']==2) { ?><a class="button dbl_target" href="index.php?do=<?php echo $do;?>&view=<?php echo $view;?>&ac=pass&code=<?php echo $code?>&record_id=<?php echo $value['mobile_a_id'];?>&obj=<?php echo $v['username']?>" 	  onclick="return cdel(this,'<?php echo $_lang['confirm_pass'];?>')"><span class="check icon"></span><?php echo $_lang['pass'];?></a><?php } ?>
<?php if($value['auth_status']==0) { ?><a class="button" href="index.php?do=<?php echo $do;?>&view=<?php echo $view;?>&code=<?php echo $code?>&ac=not_pass&record_id=<?php echo $value['mobile_a_id'];?>&obj=<?php echo $v['username']?>" onclick="return cdel(this,'<?php echo $_lang['confirm_nopass'];?>')"><span class="cross icon"></span><?php echo $_lang['nopass'];?></a><?php } ?>
-->
<?php if($value['auth_status']!=1) { ?><a class="button" href="index.php?do=<?php echo $do;?>&view=<?php echo $view;?>&code=<?php echo $code?>&ac=del&record_id=<?php echo $value['mobile_a_id'];?>&obj=<?php echo $v['username']?>" 	  onclick="return cdel(this);"><span class="trash icon"></span><?php echo $_lang['delete'];?></a><?php } ?></td>
...
<!--
<button type="submit" name="sbt_action" value="<?php echo $_lang['mulit_pass'];?>"   class="pill positive button" onclick="return batch_act(this,'frm_list');"><span class="icon fork"></span><?php echo $_lang['mulit_pass'];?></button>

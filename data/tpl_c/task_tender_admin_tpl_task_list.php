<?php keke_tpl_class::checkrefresh('task/tender/admin/tpl/task_list', '1457950005' );?><!DOCTYPE HTML>
...
<div class="page_title">
    	<h1><?php echo $model_info['model_name'];?><?php echo $_lang['manage'];?></h1>
         <div class="tool">
         	<a href="index.php?do=model&model_id=4&view=list&status=0" class="here"><?php echo $_lang['task_list'];?></a>
<!--<a href="index.php?do=model&model_id=4&view=config"><?php echo $_lang['task_config'];?></a>-->
 </div>
</div>
    <!--页头结束-->
...
<td>
                      <select name="w[ord][]">
                           <option value="task_id" <?php if($w['ord']['0']=='task_id' or !isset($w['ord']['0'])) { ?> selected="selected"<?php } ?>><?php echo $_lang['default'];?><?php echo $_lang['order'];?></option>
                           <option value="start_time" <?php if($w['ord']['0']=='start_time' ) { ?> selected="selected"<?php } ?>><?php echo $_lang['pub_time'];?></option>
                      </select>
...
<option value="30" <?php if($page_size=='30') { ?>selected="selected"<?php } ?>><?php echo $_lang['page_size'];?>30</option>
    ...
    <!--搜索结束-->
    <div class="box list">
    	<div class="title"><h2><?php echo $_lang['normal_tender'];?><?php echo $_lang['task_list'];?></h2></div>
        <div class="detail">
              ...
              <table cellpadding="0" cellspacing="0">
                <tbody>
                  <tr>
                    ...
                    <th width="22%"><?php echo $_lang['task_title'];?></th>
                    <th width="15%"><?php echo $_lang['task_status'];?></th>
                   <!-- <th width="12%"><?php echo $_lang['pub_time'];?></th>-->
                    <th width="20%"><?php echo $_lang['task_cash'];?></th>
<th width="8%"><?php echo $_lang['pub_name'];?></th>
                    <th width="30%"><?php echo $_lang['operate'];?></th>
                  </tr>
                  <?php if(is_array($task_arr)) { foreach($task_arr as $k => $v) { ?>
                  <tr class="item">
                 	<td width="10%"><?php echo $v['task_id'];?></td>
...
<?php } ?>
                     -->
                    	<?php if($v['task_status']==1||$v['task_status']==0) { ?>
                    	<a href="index.php?do=model&model_id=<?php echo $v['model_id']?>&view=list&task_id=<?php echo $v['task_id']?>&ac=pass&page=<?php echo $page;?>" onclick="return cpass(this,'',1,<?php echo $v['task_status']?>);" class="button"><span class="check icon"></span>审核通过</a>
<!-- <a href="index.php?do=model&model_id=<?php echo $v['model_id']?>&view=list&task_id=<?php echo $v['task_id']?>&ac=nopass&page=<?php echo $page;?>" onclick="return cpass(this,'',2);" class="button"><span class="cross icon"></span>审核失败</a> -->
...
art.dialog.open('index.php?do=model&model_id=4&view=list&ac=settask&task_id='+task_id,{
title:"请输入结束任务原因",
height:260,
width:450,
closeFn:function (){
...
if(ispass=='ok'){
art.dialog.data('ispass','no');
window.location="index.php?do=model&model_id=4&view=list&ac=stoptask&content="+content+"&task_id="+task_id;
}
}
},false);
}

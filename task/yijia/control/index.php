...
$taskId = intval($arrTaskInfo['task_id']);
$userInfo = keke_user_class::get_user_info($gUid);
$floatPayCash = $arrTaskInfo['task_cash'];
$boolValue = floatval($userInfo['balance'])>floatval($floatPayCash);
$sql = 'select b.order_status from '.TABLEPRE.'witkey_order_detail as a 
            left join '.TABLEPRE.'witkey_order as b 
                on a.order_id=b.order_id 
                    where a.obj_id='.$taskId.' and a.detail_type is NULL';
$hasPay = db_factory::get_one($sql);
$workIn = db_factory::query('select * from '.TABLEPRE.'witkey_task_work where task_id='.$taskId.' and work_status=5');
$workIn = count($workIn);

...
$url_str = "index.php?do=model&model_id=16&view=list&w[task_id]={$w['task_id']}&w[task_title]={$w['task_title']}&w[task_status]={$w['task_status']}&ord[0]={$ord['0']}&ord[1]={$ord['1']}&page=$page&page_size=$page_size";
        ...
        $sql = 'select b.order_status from '.TABLEPRE.'witkey_order_detail as a
            left join '.TABLEPRE.'witkey_order as b
                on a.order_id=b.order_id
                    where a.obj_id='.$task_id.' and a.detail_type is NULL';
        $hasPay = db_factory::get_one($sql);
		if ($hasPay['order_status'] == 'ok'){
    		$res = keke_finance_class::cash_in($task_audit_arr['uid'], $cash,'task_fail','','task_fail');
		}

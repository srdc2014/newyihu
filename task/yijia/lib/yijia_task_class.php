<?php
keke_lang_class::load_lang_class ( 'yijia_task_class' );
class yijia_task_class extends keke_task_class {
	...
	public static function get_instance($task_info) {
		static $obj = null;
		if ($obj == null) {
			$obj = new yijia_task_class ( $task_info );
		}
		return $obj;
	}
	...
	public function wiki_priv_init() {
		$arr = yijia_priv_class::get_priv ( $this->_task_id, $this->_model_id, $this->_userinfo );
	...
	public function work_choose($work_id, $to_status, $trust_response = false) {
		...
		if ($to_status == 4) {
			if ($this->_task_status == 7) {
				...
				if(intval($work_info['hasyj'])==3){
				    $real_cash = floatval($work_info['yjje'])*(100-$this->_task_config['task_rate'])/100;
				    $setsqlarr = array('task_cash'=>$work_info['yjje'],'real_cash'=>$real_cash);
				    $wheresqlarr = array('task_id'=>$work_info['task_id']);
				    $res = db_factory::updatetable(TABLEPRE.'witkey_task', $setsqlarr, $wheresqlarr);
				    $this->_task_info = db_factory::get_one('select * from '.TABLEPRE.'witkey_task where task_id='.intval($this->_task_id));
				}
				...
				$taskInfo = db_factory::get_one('select * from '.TABLEPRE.'witkey_task where task_id='.intval($work_info['task_id']));
				$order_body = "发布任务<a href='index.php?do=task&id=".$taskInfo['task_id']."'>" . $taskInfo['task_title'] . "</a>"; 
				$order_id = keke_order_class::create_order ( 16, $work_info['uid'], $work_info['username'], $taskInfo['task_title'], $taskInfo['task_cash'], $order_body, 'wait' );
				$order_id and keke_order_class::create_order_detail ( $order_id, $taskInfo['task_title'], 'task', intval ( $taskInfo['task_id'] ), $taskInfo['task_cash'] );
			}
		}
	...
	public function process_can() {
		...
		$process_arr['max_work'] = $config['max_work'];
		$yjtime = intval($config['yijia_end_time'])*86400;
		$process_arr ['yj_accept'] = $yjtime;
	...
	public function create_agree_date($work_info) {
		...
		$this->notify_user ( "agreement", '任务进入交付阶段', $b_arr, 2, $this->_guid );
		return $agree_id;
	}

...
class dtender_task_class extends keke_task_class {
		...
		if (true === $resText) {
			...
			if (true === $resText) {
				...
				if ($bid_id) {
					...
					for($i = 0; $i < $size; $i ++) {
						$plan_info = array (
								'plan_amount' => $plan_amount [$i],
								'plan_desc' => '',
								'plan_step' => $i + 1,
								'plan_title' => $plan_title [$i],
								'start_time' => strtotime ( $start_time [$i] ),
								'end_time' => strtotime ( $end_time [$i] ) 
						);
	...
	public function work_choose($bid_id, $to_status, $trust_response = false) {
		...
		if ($this->set_bid_status ( $bid_id, $to_status )) {
			...
			if ($to_status == 4) {
				...
				$feed_arr = array (
						"feed_username" => array (
								"content" => $bid_info ['username'],
								"url" => "index.php?do=seller&id=$bid_info[uid]" 
						),
						"action" => array (
	...
	public function plan_complete($plan_id, $plan_step) {
		...
		if (is_array ( $plan_ids ) && in_array ( $plan_id, $plan_ids )) {
			if ($this->set_plan_status ( 1, $plan_id )) {
				...
				keke_msg_class::notify_user ( $this->_guid, $this->_gusername, 'plan_confirm_pay', $_lang ['plan_confirm_pay_notice'], $v_arr, 1 );
				$objProm = keke_prom_class::get_instance ();
				$objProm->dispose_prom_event('bid_task', $uid, $obj_id);
				return true;
			} else {
	...
	public function getTuoGuanCash($task_id){
	    $sum=db_factory::get_one("select sum(quote) as sum from ".TABLEPRE."witkey_task_bid where task_id =".intval($task_id));
	   return $sum['sum'];
	}
	...
	public function plan_cash($plan_id, $title_url) {
		...
		if ($plan_info && $this->set_plan_status ( 2, $plan_id )) {
			...
			$v_arr = array (
					$_lang ['username'] => $this->_bid_info ['username'],
					$_lang ['model_name'] => $this->_model_name,
					$_lang ['task_id'] => $this->_task_id,
					$_lang ['task_title'] => $this->_task_title,
					$_lang ['num'] => $plan_info ['plan_step'],
					$_lang ['cash_msg'] => $real_cash 
			);
			keke_msg_class::notify_user ( $this->_bid_info ['uid'], $this->_bid_info ['username'], 'plan_haved_pay', $_lang ['plan_haved_pay_notice'], $v_arr, 1 );
			$feed_arr = array (
					"feed_username" => array (
							"content" => $bid_info ['username'],
							"url" => "index.php?do=seller&id=" . $bid_info ['uid'] 
					...
					"event" => array (
							"content" => "$this->_task_title",
							"url" => "index.php?do=task&id=" . $this->_task_info ['task_id'],
		...
		if ($this->_task_status == 2 && $this->_task_info ['sub_time'] < time ()) {
			...
			if ($work_info) {
				...
				$v_arr = array (
						...
						$_lang ['task_id'] => $this->_task_id,
						$_lang ['task_title'] => $this->_task_title,
						$_lang ['tb'] => $_lang ['choosing_tender'],
						'time' => date ( 'Y-m-d,H:i:s', time () ),
						'next' => $_lang ['trust'] 
				);
				keke_msg_class::notify_user ( $this->_guid, $this->_gusername, 'timeout', $_lang ['task_choose_timeout_notice'], $v_arr, 1 );
			} else {
				...
				$v_arr = array (
						...
						$_lang ['task_id'] => $this->_task_id,
						$_lang ['task_title'] => $this->_task_title,
						$_lang ['reason'] => $_lang ['tender_timeout_and_no_people_and_return_cash'],
						'explain' => '' 
				);
				keke_msg_class::notify_user ( $this->_guid, $this->_gusername, 'task_fail', $_lang ['task_fail_notice'], $v_arr, 1 );
			}
		}
	}
		...
		if ($this->_task_status == 3 && $this->_task_info ['end_time'] < time ()) {
			...
			if ($work_info) {
				...
				$v_arr = array (
						...
						$_lang ['task_id'] => $this->_task_id,
						$_lang ['task_title'] => $this->_task_title,
						'reason' => $_lang ['bid_timeout_and_you_not_choose'],
						'explain' => $_lang ['bid_timeout_and_you_not_choose_and_return'] . $res ['return_cash'] . $_lang ['yuan'] . CREDIT_NAME . ":" . $res ['return_credit'] 
				);
				keke_msg_class::notify_user ( $this->_guid, $this->_gusername, 'task_fail', $_lang ['task_fail_notice'], $v_arr, 1 );
			}
		}
	...
	public function task_tg_timeout() {
		...
		if ($this->_task_status == 4 && ! intval ( $this->_task_info ['single_cash'] ) && $this->_task_info ['sp_end_time'] < time ()) { 
			...
			$v_arr = array (
					...
					$_lang ['task_id'] => $this->_task_id,
					$_lang ['task_title'] => $this->_task_title,
					'理由' => $_lang ['trust_cash_timeout'],
					'explain' => $_lang ['trust_cash_timeout_and_return'] . $res ['return_cash'] . $_lang ['yuan'] 
			);
			keke_msg_class::notify_user ( $this->_guid, $this->_gusername, 'task_fail', $_lang ['task_fail_notice'], $v_arr, 1 );
		}
	}
	...
	public static function get_plan_status() {
		global $_lang;
		return array (
				'0' => $_lang ['wait_complete'],
				'1' => $_lang ['wait_pay'],
				'2' => $_lang ['plan_complete'] 
		);
	}
			...
			if ($leave_cash >= 0) {
				...
				foreach ( $arrOrderDetail as $k => $v ) {
					if (($v ['obj_type'] == 'task' || $v ['obj_type'] == 'hosted') && $v ['detail_type'] == null) {
						$data = array (
								$kekezu->_model_list [$task_info ['model_id']] ['model_name'],
								$task_info ['task_id'],
					...
					if ($action == 'pub_task') { 
						if ($to_status == '2') {
							...
							$v = array (
									...
									$_lang ['task_title'] => $task_info ['task_title'],
									$_lang ['task_id'] => $task_info ['task_id'],
									$_lang ['task_link'] => $url,

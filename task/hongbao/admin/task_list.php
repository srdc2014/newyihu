<?php
kekezu::admin_check_role ('m1524' );
defined ( 'ADMIN_KEKE' ) or exit ( 'Access Denied' );
$task_config = unserialize ( $model_info ['config'] );
$model_list = $kekezu->_model_list;
$task_status = hongbao_task_class::get_task_status ();
$table_obj = keke_table_class::get_instance ( 'witkey_task' );
$wh = "model_id={$model_info['model_id']}";
$page and $page=intval ( $page ) or $page = 1;
$page_size and $page_size = intval($page_size) or $page_size=10;
$url_str = "index.php?do=model&model_id=15&view=list&w[task_id]={$w['task_id']}&w[task_title]={$w['task_title']}&w[task_status]={$w['task_status']}&ord[0]={$ord['0']}&ord[1]={$ord['1']}&page=$page&page_size=$page_size";
...
switch ($ac) {
	...
	case "nopass" : 
		if($is_submit=="1"){
			...
			$strUsername=hongbao_task_class::getUsername($_GET['uid']);
			$strTittle=hongbao_task_class::getTitle($task_id);
	...
	case "refund":
		...
		$task_obj = new hongbao_task_class($task_info);

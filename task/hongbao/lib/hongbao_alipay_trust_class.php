<?php
class hongbao_alipay_trust_class {
	public $_task_id; 
	public $_interface; 
	public $_task_info; 
	public $_model_id; 
		...
		switch ($is_return) {
			...
			case true :
				switch ($this->_data ['is_success']) {
					case "T" :
						$task_obj = hongbao_task_class::get_instance ( $task_info );
			...
			case true : 
				...
				switch ($bidder_info ['transfer_status']) {
					case "W":
						...
						$agree_obj = hongbao_task_agreement::get_instance($agree_id);
			...
			case true : 
				switch($this->_data['is_success']){
					case "T":
						...
						$report_obj = hongbao_report_class::get_instance($report_id);
		...
		switch ($is_return) {
			...
			case true : 
				switch ($this->_data ['refund_detail'] ['transfer_status']) { 
					case "W" :
						$task_obj = hongbao_task_class::get_instance ( $this->_task_info );

<?php
final class hongbao_time_class extends time_base_class {
	...
	public function task_hand_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task where task_status=2 and  sub_time < '%s' and model_id = '15' and task_union!=2", TABLEPRE, time () ) );
		if (is_array ( $task_list )) {
			foreach ( $task_list as $k => $v ) {
				$task_hand_obj = new hongbao_task_class($v);
				$task_hand_obj->time_hand_end ();
			}
		}
	}
	public function task_vote_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task where task_status=4 and  sp_end_time < '%s' and model_id = '15' and task_union!=2", TABLEPRE, time () ) );
		if (is_array ( $task_list )) {
			foreach ( $task_list as $k => $v ) {
				$task_vote_obj = new hongbao_task_class( $v );
				$task_vote_obj->time_vote_end ();
			}
		}
	}
	public function task_choose_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task where task_status=3 and  end_time < '%s' and model_id = '15' and task_union!=2 ", TABLEPRE, time () ) );
		if (is_array ( $task_list )) {
			foreach ( $task_list as $k => $v ) {
				$task_choose_obj = new hongbao_task_class( $v );
				$task_choose_obj->time_choose_end ();
			}
		}
	}
	public function task_notice_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task where task_status=5 and  sp_end_time < '%s' and model_id = '15' and task_union!=2", TABLEPRE, time () ) );
		if (is_array ( $task_list )) {
			foreach ( $task_list as $k => $v ) {
				$task_notice_obj = new hongbao_task_class( $v );
	...
	public function task_top_end() {
		...
		if (is_array ( $task_list )) {
			foreach ( $task_list as $k => $v ) {
				$task_obj = new hongbao_task_class($v);

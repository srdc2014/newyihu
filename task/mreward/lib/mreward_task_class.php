...
public function getProjectProgressDesc() {
	...
	public function work_hand($work_desc, $file_ids, $hidework = '2',$work_hidden='0', $qq = '', $mobile = '') {
	...
	public function work_choose($work_id, $to_status, $trust_response = false) {
        ...
        if($to_status==7){
            $this->set_work_status ( $work_id, $to_status );
            $work_info = $this->get_task_work ( $work_id );
            $arr=array();
            $arr['用户名']=$work_info['username'];
            $arr['任务标题']=$work_info['work_title'];
            $arr['网站名称']=$kekezu->_sys_config['website_name'];
            $arr['任务编号']=$work_info['task_id'];
            $arr['稿件状态']='淘汰';
            keke_msg_class::notify_user($work_info['uid'], $work_info['username'], 'work_out', '稿件淘汰',$arr);
            return true;
        }
		...
		if($strText === true){
			...
			if ($this->set_work_status ( $work_id, $to_status )) {
				...
				if ($objProm->is_meet_requirement ( "bid_task", $this->_task_id )) {
					$res = $objProm->create_prom_event ( "bid_task", $work_info ['uid'], $this->_task_id, $prize_cash );
				}
		...
		global $kekezu;
			...
			if($arrPrizeWork){
				foreach ( $arrPrizeWork as $k => $v ) {
					...
					keke_msg_class::notify_user ( $v ['uid'], $v ['username'], 'task_bid', $status_desc_arr [$v ['work_status']], $v_arr,1 );
			...
			if (intval ( $floatSyCash ) > 0) {
				...
				keke_msg_class::notify_user ( $this->_guid, $this->_gusername, 'task_complete','自动选稿任务结束', $v_arr,1 );
			} else {
				$v_arr = array (
						$_lang ['username'] => $this->_gusername,
						$_lang ['model_name'] => $this->_model_name,
	...
	public function task_tg_timeout() {
		...
		if (time () > $this->_task_info ['sub_time'] && $this->_task_info ['task_status'] == 2) {
			...
			if ($work_num == 0) {
				...
				keke_msg_class::notify_user ( $this->_guid, $this->_gusername, 'task_fail', $_lang ['task_fail'], $v_arr ,1);
			}
			if ($work_num > 0) {
				...
				keke_msg_class::notify_user ( $this->_guid, $this->_gusername, 'timeout', $_lang ['choose_work_timeout'], $v_arr ,1);
			}
		}
	}
				...
				case "split" :
					...
					if (in_array ( $split_style, array (
						...
						foreach ( $work_bid_arr as $k => $v ) {
							...
							keke_msg_class::notify_user ( $v ['uid'], $v ['username'], 'task_bid', $status_desc_arr [$to_status], $v_arr,1 );
						}
						$this->set_task_status ( 5 );
						$this->set_task_sp_end_time ();
	...
	public function dispose_order($order_id) {
		...
		if ($order_info ['order_status'] == 'ok') {
			...
			$objProm = keke_prom_class::get_instance ();
				...
				if ($task_info ['task_cash'] < $task_config ['audit_cash']) {

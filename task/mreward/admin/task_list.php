	...
	switch ($ac) {
		...
		case "pass" : 
			...
			case "settask":
				...
				break;
			case "stoptask":
				$model_info=db_factory::get_one("select * from ".TABLEPRE."witkey_model where model_id=2");
				$config=unserialize($model_info[config]);
				$task_info = get_task_info($task_id);
				$count = array ();
				$cash = array ();
				$prize_arr = db_factory::query ( sprintf ( "select * from %switkey_task_prize where task_id='%d' ", TABLEPRE, $task_id ) );
				$i = 1;
				...
				$cash2=$cash;
				$arrworkInfo=db_factory::query("select * from ".TABLEPRE."witkey_task_work where task_id='".intval($task_id)."' and work_status in (1,2,3)");
				$workcash=0;
				if($arrworkInfo){
				...
				$work_info=db_factory::query("select * from ".TABLEPRE."witkey_task_work where task_id='".intval($task_id)."' and work_status in (1,2,3) group by uid");
				foreach($work_info as $v2){
					if($work_info){
						...
						db_factory::execute("update ".TABLEPRE."witkey_task_work set work_status=7 where task_id='".intval($task_id)."' and work_status not in (1,2,3)");
					}
				}

...
if ($is_submit == 1) {
	if (! $edituid) {
		...
		$fds ['qq'] and $arrAddUserInfo ['qq'] = $fds ['qq'];
...
if($check_email){
	$arrEmail = db_factory::get_one("select email from ".TABLEPRE."witkey_member where uid = '{$edituid}'");
	if($arrEmail['email'] == $check_email){
		echo 1;die;
	}
	$regClass = new keke_register_class();

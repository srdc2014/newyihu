...
$url = "index.php?do=$do&view=$view&w[username]=" . $w ['username'] . "&w[pay_type]=" . $w ['pay_type'] . "&w[page_size]=$page_size&w[ord]=" . $w ['ord'] . "&page=$page";
...
if (isset ( $ac )) { 
	switch ($ac) {
		case 'pass' : 
			if ($withdraw_info ['withdraw_status']) {
				if ($is_submit) {
					...
					$feed_arr = array (
							"feed_username" => array (
									"content" => $withdraw_info ['username'],
									"url" => "index.php?do=seller&id=" . $user_space_info ['uid'] 
							),
							"action" => array (
									"content" => $_lang ['withdraw'],
									"url" => "" 
							),
							"event" => array (
									"content" => $withdraw_info ['withdraw_cash'] . $_lang ['yuan'],
									"url" => "" 
							) 
					);
					...
					$v_arr = array (
							'网站名称' => $_K ['sitename'],
							'提现方式' => $pay_way [$withdraw_info ['pay_type']],
							'帐户' => $withdraw_info ['pay_account'],
							'提现金额' => $withdraw_info ['withdraw_cash'] 
					);
					keke_msg_class::notify_user ( $withdraw_info ['uid'], $withdraw_info ['username'], 'draw_success', $_lang ['withdraw_success'], $v_arr, 2 );
					...
					if (in_array ( $withdraw_info ['pay_type'], $k_arr )) {
						$withdraw_info ['auth_bank_info'] = getAuthBankInfo ( $withdraw_info ['pay_account'], $withdraw_info ['pay_type'] );
					}
				}
		...
		case 'nopass' :
			if ($withdraw_info) {
				if ($withdraw_info ['withdraw_status'] == '3') {
					kekezu::admin_show_msg ( '该提现申请已经未审核通过，无需重复操作 ', 'index.php?do=' . $do . '&view=' . $view, 3, '', 'warning' );
					die ();
				}
				$withdraw_obj->setWhere ( 'withdraw_id=' . $withdraw_id );
				...
				$data = array (
						':pay_way' => $pay_way [$withdraw_info ['pay_type']],
						':pay_account' => $withdraw_info ['pay_account'],
						':pay_name' => $withdraw_info ['pay_name'] 
				);
				...
				$v_arr = array (
						...
						'提现方式' => $pay_way [$withdraw_info ['pay_type']],
						'帐户' => $withdraw_info ['pay_account'],
						'失败原因' => $content ? kekezu::escape ( $content ) : '未注明原因',
						'提现金额' => $withdraw_info ['withdraw_cash'] 
				);
			...
			break;
		case 'ajax_html' :
			$strhtml = '<form method="post" action="?login" id="login-form">
						<p><label>拒绝的原因：<input type="text" name="username" id="login-form-username"></label></p>
						</form>';
			echo $strhtml;
			die ();
			break;
				...
				foreach ( $withdraw_arr as $k => $v ) {
					if ($v [withdraw_status] == 1) {
						...
						$feed_arr = array (
								"feed_username" => array (
										"content" => $v ['username'],
										"url" => "index.php?do=seller&id=" . $space_info ['uid'] 
								),
								"action" => array (
										"content" => $_lang ['withdraw'],
										"url" => "" 
								...
								"event" => array (
										"content" => $_lang ['withdraw_le'] . $v ['withdraw_cash'] . $_lang ['yuan'],
										"url" => "" 
								) 
						);
						kekezu::save_feed ( $feed_arr, $user_space_info ['uid'], $user_space_info ['username'], 'withdraw' );
...
function getAuthBankInfo($accout, $type) {
	return db_factory::get_one ( "SELECT * FROM `" . TABLEPRE . "witkey_auth_bank` WHERE bank_account ='" . $accout . "' AND bank_name = '" . $type . "'" );
}
